function disparity = stereo_computation(left_im, right_im, min_disp, max_disp, w_size, matching_cost, bilateral_weights)
        ...
        for j = length_side_window + 1:col - length_side_window
            ...
            % Find the ssd or ncc cost with windows at the other image
             for  k = max( j - max_disp - length_side_window, 1 + length_side_window):...
                ...
                gradIntensities = abs(left_im(i,j) - right_im(i,k));
                gradPosition = abs(j - k);
                w = exp(-(gradIntensities/gamma1 + gradPosition/gamma2));
                
                %k = max( j + min_disp, 1 + length_side_window):...
